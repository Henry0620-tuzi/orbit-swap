...
import { AlertTriangle, ArrowDownUp, CheckCircle2, ChevronDown, ExternalLink, Fuel, KeyRound, LineChart, LockKeyhole, RefreshCw, Settings2, ShieldCheck, UserRound, UsersRound, Wallet, X } from 'lucide-react'
import { encodeFunctionData, erc20Abi, formatUnits, parseUnits } from 'viem'
import { base } from 'viem/chains'
import { useAccount, useBalance, useChainId, useConnect, useDisconnect, useReadContracts, useSendTransaction, useSwitchChain, useWaitForTransactionReceipt } from 'wagmi'
...
export default function App() {
  ...
  const { data: ethBalance, isLoading: loadingBalance } = useBalance({ address, chainId: base.id, query: { enabled: isConnected && chainId === base.id } })
  const erc20Tokens = useMemo(() => TOKENS.filter(token => !native(token)), [])
  const { data: tokenBalances, isLoading: loadingTokenBalances } = useReadContracts({
    contracts: erc20Tokens.map(token => ({ address: token.address, abi: erc20Abi, functionName: 'balanceOf', args: address ? [address] : undefined })),
    query: { enabled: Boolean(address && isConnected && chainId === base.id) },
  })
  ...
  const [quoteCreatedAt, setQuoteCreatedAt] = useState<number | null>(null)
  ...
  const [quoteService, setQuoteService] = useState<'checking' | 'online' | 'offline'>('checking')
  ...
  useEffect(() => {
    setQuote(null); setQuoteCreatedAt(null); setQuoteError('')
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        setQuote(data); setQuoteCreatedAt(Date.now())
  ...
  }, [amountInUnits, sell.address, buy.address, address, slippage, quoteRefresh])
  ...
  useEffect(() => {
    const controller = new AbortController()
    fetch(`${quoteApi}/health`, { signal: controller.signal })
      .then(response => response.ok ? response.json() : Promise.reject())
      .then(data => setQuoteService(data.ok && data.configured ? 'online' : 'offline'))
      .catch(() => { if (!controller.signal.aborted) setQuoteService('offline') })
  ...
  const quoteAge = quoteCreatedAt ? Math.floor((now - quoteCreatedAt) / 1000) : 0
  const quoteExpired = Boolean(quote && quoteAge >= 20)
  const quoteSecondsLeft = Math.max(0, 20 - quoteAge)
  const wrongChain = isConnected && chainId !== base.id
  const needsApproval = Boolean(quote?.issues?.allowance && !native(sell))

  const execute = () => {
    if (!quote) return
    if (quoteExpired) { setQuoteError('报价已过期，请刷新后重新确认。'); return }
  ...
  const cta = !isConnected ? '连接钱包' : wrongChain ? '切换至 Base' : !amountInUnits ? '输入兑换数量' : loadingQuote ? '正在寻找最优报价…' : !quote ? '暂无报价' : quoteExpired ? '报价已过期，请刷新' : needsApproval ? `授权 ${sell.symbol}` : submitting ? '请在钱包中确认' : confirming ? '交易确认中…' : '确认兑换'
  ...
  const exportHistory = () => {
    ...
    const link = document.createElement('a'); link.href = url; link.download = 'orbit-swap-local-history.json'; link.click(); URL.revokeObjectURL(url)
  }
  const clearHistory = () => { localStorage.removeItem('orbit-swap-history'); setHistory([]) }
  const displayTokenBalance = (token: Token, index: number) => {
    if (wrongChain) return '—'
    if (loadingTokenBalances) return '读取中…'
    const value = tokenBalances?.[index]?.result
    if (typeof value !== 'bigint') return '0'
    return Number(formatUnits(value, token.decimals)).toLocaleString(undefined, { maximumFractionDigits: 6 })
  }

  return <main><nav><a className="brand" href="/"><span>O</span>orbit</a><div className="nav-tabs"><button className={view === 'swap' ? 'active' : ''} onClick={() => setView('swap')}>交易</button><button className={view === 'markets' ? 'active' : ''} onClick={() => setView('markets')}>市场</button><button className={view === 'portfolio' ? 'active' : ''} onClick={() => setView('portfolio')}>用户后台</button><button className={view === 'security' ? 'active' : ''} onClick={() => setView('security')}>安全中心</button><button className={view === 'admin' ? 'active' : ''} onClick={() => setView('admin')}>管理</button></div><div className={`quote-status ${quoteService}`}><b/>报价 {quoteService === 'online' ? '在线' : quoteService === 'checking' ? '检查中' : '未配置'}</div><div className="network"><b></b>Base</div>{isConnected ? <button className="wallet connected" onClick={() => disconnect()}><Wallet size={16}/>{short(address)}</button> : <button className="wallet" onClick={() => connect({ connector: connectors[0] })} disabled={connecting}><Wallet size={16}/>{connecting ? '连接中' : '连接钱包'}</button>}</nav>
    {view === 'swap' && <><div className="shell"><section className="intro"><p className="eyebrow">NON-CUSTODIAL · BASE</p><h1>让每一笔交换，<em>更清楚。</em></h1><p className="lede">一个面向链上交易者的轻量终端。Orbit 聚合 Base 上的深度，报价、路由与签名全部由你亲自确认。</p><div className="badges"><span><ShieldCheck size={16}/> 自托管钱包</span><span><LineChart size={16}/> 聚合流动性</span></div><div className="mini-stats"><span><b>1</b> 支持网络</span><span><b>5</b> 精选资产</span><span><b>0x</b> 智能路由</span></div></section>
      <section className="swap-card"><div className="swap-title"><div><p className="eyebrow">SWAP</p><h2>兑换资产</h2></div><div className="swap-actions"><button className="icon-button" aria-label="交易设置" onClick={() => setShowSettings(true)}><Settings2 size={17}/></button><span className="base-pill">Base</span></div></div>{wrongChain && <div className="notice"><AlertTriangle size={17}/> 请切换到 Base 网络后继续。</div>}
        ...
        {quote && <div className="quote-details"><div><span>最小收到（滑点 {slippage}%）</span><strong>≈ {output} {buy.symbol}</strong></div><div><span>网络费用（估算）</span><strong>{Number(formatUnits(BigInt(quote.gas) * BigInt(quote.gasPrice), 18)).toFixed(6)} ETH</strong></div><div><span>路由</span><strong>0x on Base</strong></div><div className={quoteExpired ? 'quote-expired' : ''}><span>报价有效期</span><strong>{quoteExpired ? '已过期' : `${quoteSecondsLeft} 秒`} <button className="refresh-quote" onClick={() => setQuoteRefresh(value => value + 1)} aria-label="刷新报价"><RefreshCw size={12}/></button></strong></div></div>}
        ...
        <button className="cta" onClick={onCta} disabled={loadingQuote || quoteExpired || (!isConnected && connecting) || (isConnected && !wrongChain && !quote && Boolean(amountInUnits))}>{cta}</button><p className="fineprint">{needsApproval ? '授权仅限本次输入额度；授权完成后请重新获取报价。' : '执行即表示你了解交易由钱包签名并在链上结算。请核对代币地址和钱包确认页。'}</p>
    ...
    {view === 'portfolio' && <section className="terminal"><div className="section-heading"><div><p className="eyebrow">USER CONSOLE</p><h1>用户后台</h1><p>管理钱包连接、交易活动与链上授权。</p></div><UserRound size={30}/></div>{isConnected ? <>{wrongChain && <div className="network-warning"><AlertTriangle size={18}/><span>当前钱包不在 Base 网络，无法读取 Base 余额或执行交易。</span><button onClick={() => switchChain({ chainId: base.id })}>切换到 Base</button></div>}<div className="dashboard-grid"><article className="panel balance"><span>Base 原生余额</span><strong>{wrongChain ? '—' : loadingBalance ? '读取中…' : `${Number(ethBalance?.formatted ?? 0).toFixed(5)} ETH`}</strong><small>{short(address)} · 仅从 Base 公共 RPC 读取</small><button className="wallet" onClick={() => setView('swap')} disabled={wrongChain}>开始交易</button></article><article className="panel"><div className="panel-title"><h3>本地交易记录</h3>{history.length > 0 && <span><button onClick={exportHistory}>导出</button><button onClick={clearHistory}>清除</button></span>}</div>{history.length ? history.slice(0, 3).map(transaction=><a className="list-line transaction-link" key={transaction.hash} href={`https://basescan.org/tx/${transaction.hash}`} target="_blank"><span><b>{transaction.label}</b><small>{new Date(transaction.createdAt).toLocaleString('zh-CN')}</small></span><em>{short(transaction.hash)} <ExternalLink size={12}/></em></a>) : <p>暂无本机发起的交易。连接钱包后，成功提交的交易会保存在当前浏览器。</p>}</article><article className="panel"><h3>授权管理</h3><p>Orbit 仅在交易时请求精确额度授权。请使用区块浏览器定期核查并撤销不再需要的合约授权。</p><a className="outline-link" href="https://basescan.org/tokenapprovalchecker" target="_blank">打开 BaseScan Approval Checker <ExternalLink size={13}/></a></article></div><section className="asset-section"><div><p className="eyebrow">ON-CHAIN BALANCES</p><h2>白名单资产</h2><p>仅展示 Orbit 支持的 Base 合约；数据由公开 RPC 直接读取。</p></div><div className="asset-grid"><article className="asset-card"><i style={{ background: TOKENS[0].color }}>E</i><span><b>ETH</b><small>Base 原生资产</small></span><strong>{wrongChain ? '—' : loadingBalance ? '…' : Number(ethBalance?.formatted ?? 0).toLocaleString(undefined, { maximumFractionDigits: 6 })}</strong></article>{erc20Tokens.map((token, index) => <article className="asset-card" key={token.symbol}><i style={{ background: token.color }}>{token.symbol[0]}</i><span><b>{token.symbol}</b><small>{token.name}</small></span><strong>{displayTokenBalance(token, index)}</strong></article>)}</div></section></> : <div className="portfolio-empty"><Wallet size={32}/><h3>连接钱包以进入后台</h3><p>Orbit 只读取公开链上数据，不保存任何资产或密钥。</p><button className="wallet" onClick={() => connect({ connector: connectors[0] })}>连接钱包</button></div>}</section>}
