...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!sellToken || !buyToken || sellToken === buyToken || !ALLOWED_TOKENS.has(sellToken) || !ALLOWED_TOKENS.has(buyToken) || !sellAmount || !/^\d{1,78}$/.test(sellAmount) || !Number.isInteger(slippageBps) || slippageBps < 1 || slippageBps > 1000) return json({ reason: '无效或不在白名单内的交易参数。' }, 400, headers)
    if (taker && !/^0x[a-fA-F0-9]{40}$/.test(taker)) return json({ reason: '无效的钱包地址。' }, 400, headers)
    const query = new URLSearchParams({ chainId: BASE, sellToken, buyToken, sellAmount, slippageBps: String(slippageBps) })
